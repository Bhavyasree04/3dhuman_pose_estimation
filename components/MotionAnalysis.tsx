...
const EXERCISE_IMAGES = {
  ...
  hip: {
    ...
    strengthen: 'https://images.pexels.com/photos/416778/pexels-photo-416778.jpeg', // Hip bridges
...
export const MotionAnalysis: React.FC<MotionAnalysisProps> = ({ mode, onBack }) => {
  ...
  if (loading)
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white text-amber-600 text-center">
  ...
  if (results) {
    return (
      <div className="min-h-screen bg-white text-amber-600 p-6 lg:p-12 animate-in fade-in slide-in-from-bottom-4 duration-700 font-sans">
        <div className="max-w-7xl mx-auto">
          <button onClick={onBack} className="flex items-center gap-2 text-amber-700 hover:text-amber-800 mb-8 group transition">
            <ArrowLeft className="w-5 h-5 group-hover:-translate-x-1 transition-transform" /> Back to Dashboard
          </button>

          <div className="bg-amber-50 border-2 border-amber-600 rounded-[3rem] overflow-hidden shadow-lg">
            {/* Header Area */}
            <div className="bg-amber-50 p-10 border-b-2 border-amber-600 flex flex-col md:flex-row items-center justify-between gap-8">
              <div className="text-center md:text-left">
                <h1 className="text-4xl font-serif font-black text-amber-800 mb-2">Posture Intelligence Report</h1>
                <p className="text-amber-700 opacity-80">Deep analysis of bio-mechanical joint alignment and posture performance.</p>
              </div>
              <div className="relative">
                <div className="w-32 h-32 rounded-full border-4 border-amber-600 flex flex-col items-center justify-center bg-white shadow-inner">
                  <span className="text-3xl font-black text-amber-700">{results.score}%</span>
                  <span className="text-[10px] font-bold text-amber-600 uppercase tracking-widest">Score</span>
                </div>
                <div className="absolute -bottom-2 -right-2 bg-amber-600 text-white p-1.5 rounded-full shadow-lg">
            ...
            <div className="p-10 space-y-12">
              {/* Table Section */}
              <section>
                <h3 className="text-xl font-serif font-bold text-amber-800 mb-6 flex items-center gap-2">
                  <Target className="text-amber-600 w-5 h-5" /> Joint Comparison Matrix
                </h3>
                <div className="overflow-x-auto rounded-3xl border-2 border-amber-600">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-amber-100 text-amber-800 text-xs font-black uppercase tracking-widest">
                    ...
                    <tbody className="divide-y divide-amber-600">
                     {Object.entries(HEALTHY_ANGLES).map(([key, config]) => {
                      ...
                      return (
                        <tr key={key} className="hover:bg-amber-100/50 transition-colors group">
                          <td className="px-6 py-5 font-bold text-amber-800">{config.name}</td>
                          ...
                          <td className="px-6 py-5 text-amber-700 font-mono text-sm">{config.range[0]}° - {config.range[1]}°</td>
                          <td className="px-6 py-5">
              ...
              {!showImages && (
                <div className="flex justify-center">
                  <button
                    ...
                    className="flex items-center gap-3 bg-amber-600 hover:bg-amber-700 text-white px-8 py-4 rounded-2xl font-black transition-all shadow-lg active:scale-95"
                  >
              ...
              <section className="grid lg:grid-cols-3 gap-8">
                ...
                <div className="space-y-6">
                  <h3 className="text-xl font-serif font-bold text-amber-800 flex items-center gap-2">
                    <Activity className="text-amber-600 w-5 h-5" /> Detailed Feedback
                  </h3>
                  <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2">
                    {results.feedback.map((f: FeedbackItem, i: number) => {
                      ...
                      return (
                        <div key={i} className="bg-white p-6 rounded-3xl border-2 border-amber-600 hover:border-amber-700 transition">
                          <div className="flex justify-between items-start mb-3">
                            <span className="text-xs font-black text-amber-700 uppercase">{f.joint}</span>
                          ...
                          <p className="text-amber-800 text-sm leading-relaxed mb-2">{f.message}</p>
                          {actualAngle && (
                ...
                <div className="space-y-6">
                  <h3 className="text-xl font-serif font-bold text-amber-800 flex items-center gap-2">
                    <PlayCircle className="text-amber-600 w-5 h-5" /> Video Guides
                  </h3>
                    ...
                    {results.feedback.map((f: FeedbackItem, i: number) => {
                      ...
                      return (
                        <a 
                          ...
                          className="group flex items-center justify-between p-5 bg-white hover:bg-amber-50 border-2 border-amber-600 hover:border-amber-700 rounded-3xl transition-all"
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <span className="block text-sm font-bold text-amber-800">{f.joint} {need}</span>
                              <span className="text-xs text-amber-600 italic">Watch {need} videos</span>
                            </div>
                          </div>
                          <ChevronRight className="w-5 h-5 text-amber-600 group-hover:text-amber-700 group-hover:translate-x-1 transition-all" />
                        </a>
                      );
                    })}
                  </div>
                ...
                <div className="space-y-6">
                  <h3 className="text-xl font-serif font-bold text-amber-800 flex items-center gap-2">
                    <ImageIcon className="text-amber-600 w-5 h-5" /> Personalized Exercises
                  </h3>
                  ...
                  <div className="space-y-6 max-h-[500px] overflow-y-auto pr-2">
                    {results.feedback.map((f: FeedbackItem, i: number) => {
                      ...
                      if (!showImages) {
                        return (
                          <div key={i} className="bg-white p-4 rounded-3xl border-2 border-amber-600 border-dashed">
                            <div className="flex justify-between items-center mb-3">
                              <span className="text-xs font-black text-amber-700 uppercase">{f.joint}</span>
                            </div>
                            <div className="rounded-2xl border-2 border-amber-600 mb-3 aspect-square bg-amber-100 flex items-center justify-center">
                              <ImageIcon className="w-8 h-8 text-amber-600 opacity-30" />
                            </div>
                      ...
                      return (
                        <div key={i} className="bg-white p-4 rounded-3xl border-2 border-amber-600 hover:border-amber-700 transition-all">
                          <div className="flex justify-between items-center mb-3">
                          ...
                          <div className="relative rounded-2xl overflow-hidden border-2 border-amber-600 mb-3 aspect-square bg-amber-100">
                            {!hasError ? (
                              <img 
                                src={exercise.url} 
                              ...
                              <div className="w-full h-full flex flex-col items-center justify-center p-4 bg-amber-600 text-white text-center">
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="text-[10px] text-amber-800 font-bold bg-amber-100 p-2 rounded-lg whitespace-pre-line">
                              {exercise.exercise}
                            </div>
              ...
              {/* Action Buttons */}
              <div className="flex flex-wrap justify-center gap-4 pt-10 border-t-2 border-amber-600">
                <button onClick={reset} className="flex items-center gap-3 bg-amber-600 hover:bg-amber-700 text-white px-10 py-4 rounded-2xl font-black transition-all shadow-lg active:scale-95">
                  <RefreshCcw className="w-5 h-5" /> Try Once More
                </button>
                <button onClick={onBack} className="flex items-center gap-3 bg-white hover:bg-amber-50 text-amber-700 border-2 border-amber-600 px-10 py-4 rounded-2xl font-black transition-all active:scale-95">
                  Return to Home
                </button>
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen bg-white text-amber-600 p-6 font-sans">
      <div className="max-w-7xl mx-auto">
        <button onClick={onBack} className="flex items-center gap-2 text-amber-700 hover:text-amber-800 mb-8 transition">
        ...
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className="grid md:grid-cols-2 gap-6 h-[600px]">
              {/* Camera Feed */}
              <div className="relative bg-amber-50 rounded-[2.5rem] overflow-hidden border-2 border-amber-600 shadow-lg group">
                {!analyzing && !uploadedVideo && mode === 'video' && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center z-20">
                    <Video className="w-12 h-12 text-amber-600 mb-4" />
                    <h3 className="text-xl font-black mb-2 text-amber-800">Upload File</h3>
                    <label className="bg-amber-600 hover:bg-amber-700 text-white px-8 py-3 rounded-2xl font-black cursor-pointer transition-all">
                      Browse
                ...
                <canvas 
                  ...
                  height={480} 
                />
                <div className="absolute bottom-4 left-4 z-20 bg-amber-600 text-white px-3 py-1 rounded-full text-[10px] font-bold border border-amber-300 uppercase">Camera Vision Feed</div>
              </div>
            ...
            <div className="flex items-center justify-center pt-4">
              {!analyzing ? (
                mode === 'live' ? (
                  <button onClick={startLiveAnalysis} className="flex items-center gap-4 bg-amber-600 hover:bg-amber-700 text-white px-12 py-5 rounded-3xl font-black text-xl shadow-lg transition-all hover:scale-105 active:scale-95">
                    <Camera className="w-6 h-6" /> Start 20s Live Analysis
                  </button>
                ) : uploadedVideo ? (
                  <button onClick={startVideoAnalysis} className="flex items-center gap-4 bg-amber-600 hover:bg-amber-700 text-white px-12 py-5 rounded-3xl font-black text-xl shadow-lg transition-all hover:scale-105 active:scale-95">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-amber-50 border-2 border-amber-600 rounded-[2.5rem] p-8 shadow-lg">
              <h3 className="text-xl font-serif font-black mb-6 flex items-center gap-3 text-amber-800">
                <Activity className="text-amber-600 w-6 h-6" /> Real-time Metrics
              </h3>
              <div className="space-y-6">
                  ...
                  return (
                    <div key={key} className="space-y-2">
                      <div className="flex justify-between text-[10px] font-black uppercase tracking-widest">
                        <span className="text-amber-700">{info.name}</span>
                        <span className={status === 'optimal' ? 'text-emerald-700' : status === 'warning' ? 'text-amber-600' : 'text-rose-600'}>
                          {Math.round(val)}°
                        </span>
                      </div>
            ...
            <div className="bg-amber-50 border-2 border-amber-600 rounded-[2rem] p-6">
              ...
              <p className="text-[10px] text-amber-700 leading-relaxed uppercase tracking-wider font-bold">
